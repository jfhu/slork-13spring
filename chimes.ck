...
fun void play_keystroke(int ascii) {
    ...
    Math.random2(0,2) => int rand;
    if (rand == 0)
        "sample/chimes/chimes_double.wav" => buf.read;
    else if (rand == 1)
        "sample/chimes/chimes_single.wav" => buf.read;
    else
        "sample/chimes/chimes_short_asc.wav" => buf.read;
    //rate => buf.rate;
    5 => buf.gain;
    0 => buf.pos;
    buf.length() => now;
...
fun void play_forward() {
    ...
    Math.random2(0,2) => int rand;
    if (rand == 0)
        "sample/chimes/chimes_multiple_same.wav" => buf.read;
    else
        "sample/chimes/chimes_down_up.wav" => buf.read;
    0 => buf.pos;
    5 => buf.gain;
    buf.length() => now;
}
...
fun void play_space() {
    SndBuf buf => ADSR env => dac;
    env.set(30::ms, 50::ms, .1, 50::ms);
    "sample/chimes/chimes_desc.wav" => buf.read;
    0 => buf.pos;
    5 => buf.gain;
    buf.length() => now;
}
